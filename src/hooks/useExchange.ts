...
interface UseExchangeReturn {
  ...
  // Akcje dla ofert
  createOffer: (book: Book, condition: BookCondition, exchangeType: ExchangeType, description?: string, location?: string) => Promise<boolean>;
  updateOffer: (offerId: string, updates: Partial<ExchangeOffer>) => Promise<boolean>;
  ...
  sendMessage: (offerId: string, recipientId: string, message: string) => Promise<boolean>;
...
export const useExchange = (bookId?: string): UseExchangeReturn => {
  ...
  // Stan dla transakcji
  const [userTransactions, setUserTransactions] = useState<ExchangeTransaction[]>([]);
  const [activeTransactions, setActiveTransactions] = useState<ExchangeTransaction[]>([]);
    ...
    try {
      ...
      if (data) {
        const offers = data.map(offer => ({
          ...offer,
          book_data: typeof offer.book_data === 'string' ? JSON.parse(offer.book_data) : offer.book_data
        })) as ExchangeOffer[];
      ...
      setOffersError('Nie udało się pobrać ofert wymiany. Spróbuj ponownie później.');
    } finally {
  ...
  const fetchUserOffers = useCallback(async () => {
    ...
    try {
      ...
      setOffersError('Nie udało się pobrać Twoich ofert. Spróbuj ponownie później.');
    } finally {
      setIsLoadingOffers(false);
    }
  ...
  const fetchOfferDetails = useCallback(async (offerId: string): Promise<ExchangeOffer | null> => {
    ...
    try {
      ...
      if (data) {
        const offer = {
          ...data,
          book_data: typeof data.book_data === 'string' ? JSON.parse(data.book_data) : data.book_data
        } as ExchangeOffer;
      ...
      setOffersError('Nie udało się pobrać szczegółów oferty. Spróbuj ponownie później.');
      return null;
    } finally {
      setIsLoadingOffers(false);
    }
  ...
  const createOffer = async (
    ...
    try {
      ...
      setOffersError('Nie udało się dodać oferty wymiany. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingOffers(false);
    }
  ...
  // Aktualizacja oferty
  const updateOffer = async (offerId: string, updates: Partial<ExchangeOffer>): Promise<boolean> => {
    if (!isAuthenticated || !user) {
      setOffersError('Musisz być zalogowany, aby zaktualizować ofertę wymiany.');
      return false;
    }
    ...
    try {
      ...
      const { id, user_id, book_id, book_data, created_at, user_details, ...updateData } = updates;
      ...
      setOffersError('Nie udało się zaktualizować oferty wymiany. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingOffers(false);
    }
  ...
  const deleteOffer = async (offerId: string): Promise<boolean> => {
    ...
    try {
      ...
      setOffersError('Nie udało się usunąć oferty wymiany. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingOffers(false);
    }
  ...
  const setOfferActive = async (offerId: string, active: boolean): Promise<boolean> => {
  ...
  const fetchOfferMessages = useCallback(async (offerId: string) => {
    ...
    try {
      ...
      setMessagesError('Nie udało się pobrać wiadomości. Spróbuj ponownie później.');
    } finally {
  ...
  const sendMessage = async (offerId: string, recipientId: string, message: string): Promise<boolean> => {
    if (!isAuthenticated || !user) {
    ...
    try {
      ...
      setMessagesError('Nie udało się wysłać wiadomości. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingMessages(false);
    }
  ...
  const markMessageAsRead = async (messageId: string): Promise<boolean> => {
    if (!isAuthenticated || !user) {
      setMessagesError('Musisz być zalogowany, aby oznaczyć wiadomość jako przeczytaną.');
      return false;
    }
    ...
    try {
      ...
      if (data) {
        ...
        const transactions = data.map(transaction => {
          if (transaction.offer_details && transaction.offer_details.book_data) {
            return {
              ...transaction,
              offer_details: {
                ...transaction.offer_details,
                book_data: typeof transaction.offer_details.book_data === 'string'
      ...
      setTransactionsError('Nie udało się pobrać transakcji. Spróbuj ponownie później.');
    } finally {
  ...
  const requestExchange = async (offerId: string, ownerId: string): Promise<boolean> => {
    if (!isAuthenticated || !user) {
      setTransactionsError('Musisz być zalogowany, aby złożyć prośbę o wymianę.');
      return false;
    }
    
    if (!offerId || !ownerId) {
      setTransactionsError('Brak wymaganych danych do złożenia prośby o wymianę.');
      return false;
    }
    ...
    try {
      ...
      if (existingTransaction) {
        setTransactionsError('Już złożyłeś prośbę o wymianę tej książki. Poczekaj na odpowiedź właściciela.');
        return false;
      }
      ...
      if (data) {
        ...
        const newTransaction = {
          ...
          offer_details: data.offer_details && {
            ...data.offer_details,
            book_data: typeof data.offer_details.book_data === 'string'
      ...
      setTransactionsError('Nie udało się złożyć prośby o wymianę. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingTransactions(false);
    }
  ...
  const updateTransactionStatus = async (transactionId: string, status: TransactionStatus): Promise<boolean> => {
    if (!isAuthenticated || !user) {
      setTransactionsError('Musisz być zalogowany, aby zaktualizować status transakcji.');
      return false;
    }
    ...
    try {
      ...
      if (data) {
        ...
        if (
          ...
          setActiveTransactions(prev => prev.filter(tr => tr.id !== transactionId));
        } else {
      ...
      setTransactionsError('Nie udało się zaktualizować statusu transakcji. Spróbuj ponownie później.');
      return false;
    } finally {
      setIsLoadingTransactions(false);
    }
