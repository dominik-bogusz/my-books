...
const ReadingStats: React.FC<ReadingStatsProps> = ({ stats, isLoading, error }) => {
  if (isLoading) {
    return (
  ...
  const daysInYear = new Date(currentYear, 11, 31).getDate() + 365 - new Date(currentYear, 11, 31).getDay();
  const dayOfYear = Math.floor((new Date().getTime() - new Date(currentYear, 0, 1).getTime()) / (24 * 60 * 60 * 1000)) + 1;
        ...
        <div className="col">
          <div className="card h-100">
            <div className="card-body text-center">
              ...
              <p className="text-muted">Najdłuższa: {stats.longest_streak_days} dni</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="card-body">
          <div className="d-flex align-items-center">
            ...
            <div>
              ...
              <p className="text-muted">Średni czas potrzebny na przeczytanie książki</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          {stats.favorite_genres.length > 0 ? (
                  ...
                  <div key={index} className="mb-3">
                    ...
                    <div className="progress" style={{ height: '10px' }}>
                      <div 
                        ...
                        role="progressbar" 
                        style={{ 
                          width: `${(genre.count / stats.favorite_genres[0].count) * 100}%`,
                          background: 'linear-gradient(90deg, #DA831C 0%, #FFD028 100%)'
                        }} 
                        aria-valuenow={(genre.count / stats.favorite_genres[0].count) * 100} 
                        aria-valuemin={0} 
                        aria-valuemax={100}
                      ></div>
                    </div>
              ...
              <div className="col-md-4">
                <div className="d-flex justify-content-center align-items-center h-100">
                  <div className="text-center">
                    <i className="fas fa-book-open fa-3x mb-3" style={{ color: '#DA831C' }}></i>
                    <p className="mb-0">
                      Twój ulubiony gatunek to <br />
                      <strong>{stats.favorite_genres[0]?.genre || "Brak danych"}</strong>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <p className="text-center text-muted">Brak danych o ulubionych gatunkach.</p>
          )}
        </div>
      </div>
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          {stats.reading_by_month.length > 0 ? (
            <div className="chart-container" style={{ height: '300px' }}>
              ...
              <div className="d-flex align-items-end justify-content-between h-100">
                {stats.reading_by_month.slice(-12).map((month, index) => (
                  <div key={index} className="d-flex flex-column align-items-center" style={{ width: `${100 / Math.min(stats.reading_by_month.length, 12)}%` }}>
                    <div 
                      className="w-75" 
                      style={{ 
                        height: `${(month.count / Math.max(...stats.reading_by_month.map(m => m.count))) * 200}px`,
                        background: 'linear-gradient(180deg, #DA831C 0%, #FFD028 100%)',
                        borderRadius: '4px 4px 0 0'
                      }}
                    ></div>
                    <div className="text-center mt-2" style={{ fontSize: '0.8rem' }}>
                      <div>{month.count}</div>
                      <div className="text-muted">{formatMonthName(month.month)}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <p className="text-center text-muted">Brak danych o aktywności czytelniczej.</p>
          )}
        </div>
      </div>
